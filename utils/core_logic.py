...
NOVEL_WORLDS = {
    "暗影纪元": {
        ...
        "system_prompt": (
            ...
            "角色设定：\n"
            "- 你同时扮演多个NPC角色，每个角色有独特的说话风格\n"
            "- 你是公正的旁白，描述场景、氛围和NPC的反应\n"
            "- 当玩家做出选择时，描述后果并推进剧情\n\n"
            "叙事规则：\n"
            "1. 每次回复要生动、有画面感，使用适当的环境描写\n"
            "2. 根据玩家的生命值和物品调整难度和奖励\n"
            "3. 适时引导玩家使用工具（查看线索、战斗、休息等）\n"
            "4. 保持剧情连贯性，记住之前发生的重要事件\n"
            "5. 使用Markdown格式，适当使用**加粗**强调重要信息\n\n"
            "重要：你是游戏世界的一部分，用第二人称「你」来描述玩家的行动和遭遇。"
        ),
        "starting_clues": [
    ...
    "仙途": {
        ...
        "system_prompt": (
            ...
            "4. 保持修仙世界观的一致性\n"
            "5. 使用Markdown格式增强可读性\n\n"
            "重要：你是修仙世界的一部分，用第二人称「你」来描述玩家的行动和遭遇。"
        ),
        "starting_clues": [
...
def start_new_game(user_id: str, novel_title: str) -> int:
    db_manager.create_user(user_id, user_id)
    session_id = db_manager.create_session(user_id, novel_title)
    ...
    intro = (
        f"=== 欢迎来到《{novel_title}》===\n\n"
        "你睁开眼睛，发现自己置身于一个陌生的世界。空气中弥漫着未知的气息，远处传来隐约的声响...\n\n"
        "你现在是一名初入江湖的冒险者，前方的道路充满未知。你可以：\n"
        "- 输入「探索」向前探索未知区域\n"
        "- 输入「对话」寻找附近的NPC交谈\n"
        "- 输入「战斗」主动寻找敌人\n"
        "- 输入「查看线索」了解已收集的信息\n"
    ...
    db_manager.save_chat(session_id, "assistant", intro)
