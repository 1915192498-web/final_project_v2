...
NOVEL_WORLDS = {
    "暗影纪元": {
        ...
        "system_prompt": (
            ...
            "6. 使用Markdown格式，适当使用**加粗**强调重要信息\n\n"
            "前后一致性规则（最高优先级）：\n"
            "- 绝对不能与之前发生过的事件矛盾\n"
            "- NPC的名字、身份、说过的话必须前后一致\n"
            "- 玩家的物品、属性、位置必须准确记录\n"
            "- 如果玩家之前击败了某个敌人，不能让他再次出现\n"
            "- 如果玩家获得了某个物品或线索，后续剧情必须承认\n"
            "- 地点和场景转换必须合理，不能突然瞬移\n\n"
            "重要：\n"
...
def build_novel_chain(novel_title: str):
    ...
    def invoke_chain(input_data: dict) -> str:
        ...
        raw_history = db_manager.get_chat_history(session_id, limit=50)
